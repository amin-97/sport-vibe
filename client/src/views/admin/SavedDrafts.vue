<!-- src/views/admin/SavedDrafts.vue -->
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white rounded-lg shadow p-6">
      <h1 class="text-2xl font-bold text-gray-900 mb-6">Saved Drafts</h1>
      ...
      <div v-else class="overflow-x-auto">
        ...
        <div class="mb-4 text-sm text-gray-500">
          Total drafts: {{ filteredAndSortedItems.length }}
        </div>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-if="filteredAndSortedItems.length === 0">
              <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                No drafts found for {{ currentTab }}
              </td>
            </tr>
...
const filteredAndSortedItems = computed(() => {
  let validItems =
    items.value?.filter((item) => item && (item.title || item.name) && item.status === 'draft') ||
    []
