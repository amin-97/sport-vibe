<!-- src/views/admin/CreateNBAEditorial.vue -->
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white rounded-lg shadow p-6">
      <h1 class="text-2xl font-bold text-gray-900 mb-6">Create NBA Editorial</h1>
...
const handleSubmit = async () => {
  try {
    ...
    const response = await axios.post('/api/nba/editorials', formData, {
      headers: {
    ...
    router.push('/nba/editorials')
  } catch (error) {
    console.error('Error creating editorial:', error)
    alert(error.response?.data?.message || 'Error creating editorial')
  } finally {
